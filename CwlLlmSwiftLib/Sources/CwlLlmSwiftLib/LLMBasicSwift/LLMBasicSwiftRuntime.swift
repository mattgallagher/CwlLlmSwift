...
private struct LLMBasicSwiftTokenDataLoader {
    ...
    init(data: Data, batchSize: Int, sequenceLength: Int) throws {
        ...
        guard data.count >= headerBytes else {
            throw LLMBasicSwiftRuntimeError.invalidDataset("Dataset shard is too small to contain a valid header.")
        }
        ...
        guard header.count >= 3 else {
            throw LLMBasicSwiftRuntimeError.invalidDataset("Dataset shard header is incomplete.")
        }
        guard header[0] == Self.magic else {
            throw LLMBasicSwiftRuntimeError.invalidDataset("Dataset shard has invalid magic value \(header[0]).")
        }
        guard header[1] == Self.version else {
            throw LLMBasicSwiftRuntimeError.invalidDataset("Dataset shard uses unsupported version \(header[1]).")
        }
        ...
        guard tokens.count == tokenCount else {
            throw LLMBasicSwiftRuntimeError.invalidDataset("Dataset shard token count mismatch: header says \(tokenCount), file contains \(tokens.count).")
        }
        guard tokenCount >= batchSize * sequenceLength + 1 else {
            throw LLMBasicSwiftRuntimeError.invalidDataset("Dataset shard does not contain enough tokens for a batch of size \(batchSize) and sequence length \(sequenceLength).")
        }
...
actor LLMBasicSwiftRuntime {
    private var model: LLMBasicSwift.GPT2?
    ...
    func loadCheckpoint(data: Data) async throws {
        ...
        model = try LLMBasicSwift.buildModel(from: data)
    }
    ...
    func startInference(request: LLMInferenceRequest) async throws -> AsyncThrowingStream<LLMInferenceChunk, Error> {
        ...
        try ensureModel()
        guard let model else {
            throw LLMBasicSwiftRuntimeError.failedToCreateModel
        }
    ...
    private func trainingLoop(request: LLMTrainingRequest, continuation: AsyncThrowingStream<LLMTrainingProgress, Error>.Continuation) async throws {
        ...
        var trainLoader = try LLMBasicSwiftTokenDataLoader(data: trainData, batchSize: batchSize, sequenceLength: min(sequenceLength, model.config.max_seq_len))
        var validationLoader = try LLMBasicSwiftTokenDataLoader(data: validationData, batchSize: batchSize, sequenceLength: min(sequenceLength, model.config.max_seq_len))
        ...
        for _ in 0..<request.stepCount {
            ...
            LLMBasicSwift.gpt2_forward(model: &model, inputs: trainLoader.inputs, targets: trainLoader.targets, B: batchSize, T: effectiveSequenceLength)
    ...
    private func recreateModel() throws {
        ...
        model = try LLMBasicSwift.buildModel(from: currentCheckpointData)
    }
    ...
    private func computeValidationLoss(using model: inout LLMBasicSwift.GPT2, loader: inout LLMBasicSwiftTokenDataLoader, batchCount: Int, sequenceLength: Int) throws -> Double? {
