...
This version is the clean, minimal Swift port of the C reference. As such:
- it runs on CPU.
- it does not make the code too complex; it is readable.
- it stays close to the C reference implementation where practical.
...
enum LLMBasicSwift {
    ...
    static func encoder_forward(out: inout [Float], inp: [UInt32], wte: [Float], wpe: [Float], B: Int, T: Int, C: Int) {
        ...
        for b in 0..<B {
            for t in 0..<T {
                ...
                for i in 0..<C {
                    out[out_bt + i] = wte[wte_ix + i] + wpe[wpe_t + i]
                }
            }
        }
    }
                ...
                for h in 0..<NH {
                    ...
                    for t2 in 0...t {
                        ...
                        for i in 0..<hs {
                            ...
                            // so now we have:
                            datt[datt_bth + t2] += inp[value_t2 + i] * dout[dout_bth + i]
    ...
    static func gpt2_update_field(params: inout [Float], grads: [Float], m_memory: inout [Float], v_memory: inout [Float], update_params: UpdateParams) {
        for i in 0..<params.count {
            ...
            let m = update_params.beta1 * m_memory[i] + (1 - update_params.beta1) * grad
            // update the second moment (RMSprop)
            let v = update_params.beta2 * v_memory[i] + (1 - update_params.beta2) * grad * grad
