...
enum LLMBLAS {
    ...
    static func matmul_forward(out: inout [Float], inp: [Float], weight: [Float], bias: [Float]?, B: Int, T: Int, C: Int, OC: Int) {
        ...
        for bt in 0..<(B * T) {
            cblas_saxpy(Int32(OC), 1, &bias, 1, &out[bt * OC], 1)
        }
    }
    ...
    static func matmul_backward(dinp: inout [Float], dweight: inout [Float], dbias: inout [Float], dout: [Float], inp: [Float], weight: [Float], B: Int, T: Int, C: Int, OC: Int) {
        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Int32(C), Int32(B * T), Int32(OC), 1, weight, Int32(C), dout, Int32(OC), 1, &dinp, Int32(C))
        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasTrans, Int32(C), Int32(OC), Int32(B * T), 1, inp, Int32(C), dout, Int32(OC), 1, &dweight, Int32(C))

        guard !dbias.isEmpty else {
            return
        }
        dout.withUnsafeBufferPointer { doutBuffer in
            guard let doutBase = doutBuffer.baseAddress else {
                return
            }
            for bt in 0..<(B * T) {
                cblas_saxpy(Int32(OC), 1, doutBase.advanced(by: bt * OC), 1, &dbias, 1)
            }
        }
    }
    ...
    static func attention_forward(out: inout [Float], preatt: inout [Float], att: inout [Float], inp: [Float], B: Int, T: Int, C: Int, NH: Int) {
        ...
        for b in 0..<B {
            for h in 0..<NH {
                ...
                preatt.withUnsafeMutableBufferPointer { buffer in
                    guard let baseAddress = buffer.baseAddress?.advanced(by: attBase) else {
                        return
                    }
                    cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans, Int32(T), Int32(T), Int32(headSize), 1, k[iterationIndex], Int32(headSize), q[iterationIndex], Int32(headSize), 0, baseAddress, Int32(T))
                }
                ...
                vAccum[iterationIndex].withUnsafeMutableBufferPointer { accumBuffer in
                    guard let accumBase = accumBuffer.baseAddress else {
                        return
                    }
                    att.withUnsafeBufferPointer { attBuffer in
                        guard let attBaseAddress = attBuffer.baseAddress?.advanced(by: attBase) else {
                            return
                        }
                        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Int32(headSize), Int32(T), Int32(T), 1, v[iterationIndex], Int32(headSize), attBaseAddress, Int32(T), 0, accumBase, Int32(headSize))
                    }
                    ...
                    for t in 0..<T {
                        for hs in 0..<headSize {
                            ...
                            out[outIndex] = accumBase[valueIndex]
                        }
                    }
                }
            }
    ...
    static func attention_backward(dinp: inout [Float], dpreatt: inout [Float], datt: inout [Float], dout: [Float], inp: [Float], att: [Float], B: Int, T: Int, C: Int, NH: Int) {
        ...
        for b in 0..<B {
            for h in 0..<NH {
                ...
                let attBase = b * NH * T * T + h * T * T
                datt.withUnsafeMutableBufferPointer { buffer in
                    guard let baseAddress = buffer.baseAddress?.advanced(by: attBase) else {
                        return
                    }
                    cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans, Int32(T), Int32(T), Int32(headSize), 1, v[iterationIndex], Int32(headSize), dvAccum[iterationIndex], Int32(headSize), 0, baseAddress, Int32(T))
                }
                ...
                att.withUnsafeBufferPointer { attBuffer in
                    ...
                    dv[iterationIndex].withUnsafeMutableBufferPointer { dvBuffer in
                        guard let dvBase = dvBuffer.baseAddress else {
                            return
                        }
                        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasTrans, Int32(headSize), Int32(T), Int32(T), 1, dvAccum[iterationIndex], Int32(headSize), attBaseAddress, Int32(T), 0, dvBase, Int32(headSize))
                    }
                }
                ...
                dpreatt.withUnsafeBufferPointer { preattBuffer in
                    guard let preattBaseAddress = preattBuffer.baseAddress?.advanced(by: attBase) else {
                        return
                    }
                    dq[iterationIndex].withUnsafeMutableBufferPointer { dqBuffer in
                        guard let dqBase = dqBuffer.baseAddress else {
                            return
                        }
                        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Int32(headSize), Int32(T), Int32(T), 1, k[iterationIndex], Int32(headSize), preattBaseAddress, Int32(T), 0, dqBase, Int32(headSize))
                    }
                    dk[iterationIndex].withUnsafeMutableBufferPointer { dkBuffer in
                        guard let dkBase = dkBuffer.baseAddress else {
                            return
                        }
                        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasTrans, Int32(headSize), Int32(T), Int32(T), 1, q[iterationIndex], Int32(headSize), preattBaseAddress, Int32(T), 0, dkBase, Int32(headSize))
                    }
                }
